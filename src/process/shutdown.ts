 ...
 *    which is exactly what the force-kill step below uses when `killTree` is
 *    enabled.
 * 2. **There is no true graceful signal on Windows.** Node maps any signal
 ...
 *    code. The classic "signal, wait, force-kill" two-step is still
 *    implemented here (see below for why), but do not read a Windows
 *    `exitedOnSignal` result as "the process cleaned up after itself" the way
 *    a POSIX SIGTERM handler might have.
 *
 *    **What `graceMs` actually buys on Windows, stated plainly:** not
 *    cleanup time — that opportunity never existed. What it buys is (a) a
 *    bounded wait for the OS to actually finish tearing the process down and
 *    release its handles before this module gives up and reaches for the
 *    heavier `taskkill /T /F` hammer, and (b) the ability to skip that heavier
 *    step entirely for the common case where `TerminateProcess`-of-the-parent
 *    alone was already sufficient (a process with no children of its own).
 *    On POSIX, by contrast, `graceMs` is the classic meaningful window: SIGTERM
 *    is a real, catchable signal, and a well-behaved child can close sockets,
 *    flush files, and exit on its own before `SIGKILL` ever becomes necessary.
 *    A doc comment claiming Windows gets the same benefit would be a fiction;
 *    this one does not make that claim.
 ...
 *
 * ## A known, deliberate gap: a gracefully-exiting parent's orphans
 *
 * `killTree`/`taskkill` is only reached on the force-kill (escalation) path.
 * If a process exits on its own within `graceMs` (`exitedOnSignal`), this
 * module does **not** also taskkill its process tree to mop up any
 * grandchildren it may have left behind — it takes the parent's own exit at
 * face value. Two reasons: it is not this module's job to second-guess a
 * process that exited the way it was asked to, and — more concretely on
 * Windows — running `taskkill /PID <pid>` against a PID that has *already*
 * exited risks targeting a PID Windows has since recycled for an unrelated
 * process, which would be actively dangerous, not merely unnecessary. If a
 * supervised process spawns children that must not outlive it, that process
 * needs to be responsible for them itself (or be wrapped in a job object
 * upstream of this module) — `shutdownAll`'s tree-kill guarantee applies only
 * to the force-kill path, where the target pid is one this module itself just
 ...
 * itself could not even be issued (e.g. `taskkill.exe` missing).
 */
