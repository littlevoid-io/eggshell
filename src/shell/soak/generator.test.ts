...
describe('ActionGenerator', () => {
  it('produces identical action sequence for identical seeds', () => {
    ...
    const actionsA = Array.from({ length: 15 }, (_, i) => generatorA.nextStep('win-1', 1000 + i));
    const actionsB = Array.from({ length: 15 }, (_, i) => generatorB.nextStep('win-1', 1000 + i));
  ...
  it('diverges when initialized with different seeds', () => {
    ...
    const actionsA = Array.from({ length: 10 }, (_, i) => generatorA.nextStep('win-1', 1000 + i));
    const actionsB = Array.from({ length: 10 }, (_, i) => generatorB.nextStep('win-1', 1000 + i));
  ...
  it('respects actionTypes filter to generate only specified types', () => {
    ...
    const actions = Array.from({ length: 20 }, (_, i) => generator.nextStep('win-1', 2000 + i));
  ...
  it('increments step counter and records windowId and timestamp', () => {
    ...
    const a1 = generator.nextStep('test-win', 5000);
    const a2 = generator.nextStep('test-win', 6000);
  ...
  it('constrains coordinate generation to configured viewport', () => {
    ...
    for (let i = 0; i < 50; i++) {
      const action = generator.nextStep('win-1', 1000 + i);
  ...
  it('constrains coordinate generation to per-call viewport bounds', () => {
    ...
    for (let i = 0; i < 50; i++) {
      const action = generator.nextStep('win-1', 1000 + i, 800, 600);
  ...
  it('produces identical action sequence for identical seeds with per-call viewport', () => {
    ...
    const actionsA = Array.from({ length: 15 }, (_, i) =>
      generatorA.nextStep('win-1', 1000 + i, 800, 600)
    );
    const actionsB = Array.from({ length: 15 }, (_, i) =>
      generatorB.nextStep('win-1', 1000 + i, 800, 600)
    );
