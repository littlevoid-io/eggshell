...
describe('dashboard auth (T4.2)', () => {
  ...
  describe('extractRequestToken', () => {
    it('extracts token from Bearer authorization header', () => {
      ...
      expect(extractRequestToken(req)).toBe('my-bearer-token');
    });
    ...
    it('extracts token from x-dashboard-token header', () => {
      ...
      expect(extractRequestToken(req)).toBe('header-token');
    });
    ...
    it('extracts token from query parameter', () => {
      ...
      expect(extractRequestToken(req)).toBe('query-token');
    });

    it('returns undefined when no token is present', () => {
      const req = { headers: {}, query: {} } as unknown as Request;
      expect(extractRequestToken(req)).toBeUndefined();
    });
  });
  ...
  describe('createAuthMiddleware', () => {
    ...
    it('rejects with 401 when expected token is not provided', () => {
