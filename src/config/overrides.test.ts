...
import { DEFAULT_OVERRIDE_FILENAME, loadExhibitConfig, resolveOverridePath } from './overrides.js';
...
describe('loadExhibitConfig — no override file', () => {
  it('returns the validated code config and logs that no override file was found', () => {
    const { logger, calls } = createCapturingLogger();
    const result = loadExhibitConfig({ config: minimalConfig(), roots, logger });

    expect(result.productName).toBe('Minimal Exhibit');
...
describe('loadExhibitConfig — scalar override', () => {
  it('applies a valid override that changes a scalar field, and logs the applied path', () => {
    const overridePath = writeOverride(JSON.stringify({ productName: 'Deployed Exhibit' }));
    const { logger, calls } = createCapturingLogger();

    const result = loadExhibitConfig({ config: minimalConfig(), roots, logger });

    expect(result.productName).toBe('Deployed Exhibit');
    const infoCall = calls.find(call => call.level === 'info');
...
describe('loadExhibitConfig — nested object merge', () => {
  it('merges a nested field without clobbering its siblings', () => {
    ...
    const result = loadExhibitConfig({ config, roots });
...
describe('loadExhibitConfig — array replacement', () => {
  it('replaces the windows array wholesale rather than merging it', () => {
    ...
    const result = loadExhibitConfig({ config: minimalConfig(), roots });
...
describe('loadExhibitConfig — non-object top-level value', () => {
...
describe('loadExhibitConfig — override introduces an invalid enum', () => {
...
describe('loadExhibitConfig — override introduces an unknown key', () => {
...
describe('loadExhibitConfig — prototype pollution guard', () => {
...
describe('resolveOverridePath — explicit deploymentOverridePath', () => {
  it('honours an explicit relative path instead of the userData default', () => {
    fs.writeFileSync(
      ...
      JSON.stringify({ productName: 'Custom Path Exhibit' }),
      'utf8'
    );
    ...
    expect(result.productName).toBe('Custom Path Exhibit');
  });
...
describe('loadExhibitConfig — input immutability', () => {
  it('does not mutate the code config or the override file contents', () => {
    ...
    loadExhibitConfig({ config, roots });
