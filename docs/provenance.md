...
| `scripts/build.ts` (electron-builder per app)                       | `eggshell build`: staged app folder + electron-builder (`src/build/`)       | rewritten | `release/win-unpacked/<Product>.exe` runs                        |
| `scripts/startPackaged.ts`                                          | `eggshell start` (`src/cli/commands/start.ts`), launch manifest             | rewritten | Launches built exe, exits with app exit code                     |
...
| Window icon (`iconPath`, `public/ui/icon.png`) | `icon` / `windows[].icon` (window + electron-builder icon)               | rewritten | Taskbar and exe show the app icon                            |
