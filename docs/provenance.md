...
| ----------------------------------------------- | -------------------------------------- | --------- | ----------------------------------------------------------------- |
| `electron.config.ts` factory `({appDir,isDev})` | `eggshell.config.ts` factory           | todo      | `eggshell doctor` prints resolved config; typo yields field path  |
...
| `scripts/dev.ts` (tsc watch + Vite + Electron)  | `eggshell dev`                         | todo      | Vite process starts, window loads after readiness                 |
...
| `PARENT_PID` watchdog (quit when runner dies)   | `src/shell/lifecycle`                  | todo      | Kill `eggshell dev` terminal; Electron exits within 2 s          |
| `applyExhibitFlags` (Chromium command-line switches) | `src/shell/chromium-flags.ts`     | todo      | `force-device-scale-factor=1` visible in `chrome://version`      |
| `keepDisplayAwake` (powerSaveBlocker)           | `src/shell/lifecycle`                  | todo      | Display stays on past OS sleep timeout                            |
...
| `remote-debugging-port=9223` in dev             | `src/shell/chromium-flags.ts`          | todo      | `chrome://inspect` finds the window                               |

## Windows and layout

| Original                                        | eggshell                               | Status    | Test                                                              |
| ----------------------------------------------- | -------------------------------------- | --------- | ----------------------------------------------------------------- |
| `WindowManager` create/lock/focus               | `src/shell/windows`                    | rewritten | Kiosk window opens, always-on-top, no context menu                |
...
| Original                                        | eggshell                               | Status    | Test                                                              |
| ----------------------------------------------- | -------------------------------------- | --------- | ----------------------------------------------------------------- |
| pino + pino-roll rotating file log              | `logging` section, `src/logging/file`  | todo      | Log file rotates at configured size; old files pruned             |
| chalk per-module colored terminal output        | `src/cli/output`                       | todo      | Dev terminal shows `[shell]`, `[layout]` in distinct colors       |
| Log listener fan-out (dashboard stream)         | `src/logging` listener                 | todo      | Dashboard log console updates live                                |
| `FORCE_COLOR=1`                                 | `src/cli/output`                       | todo      | Colors survive when Electron is a child process                   |
