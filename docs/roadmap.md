...
| 10  | Ship the Windows touch probe at all? Post-T2.12 it can only break a tie when Electron reports `'unknown'`, and its ids are unverified WMI ordinals                                                                          | **Resolved by research (T7.1).** No Windows API correlates a WMI `Win32_PointingDevice` ordinal to a display — the real OS mapping mechanism (Container ID / ACPI `_PLD`) is unrelated to WMI enumeration order, and even the correct application-facing API (`POINTER_DEVICE_INFO.hmonitor`) is native-only and documented by Microsoft as unreliable. Keep the probe, keep it `enabled: false` by default, do not invest further engineering into it — not worth trading this project's zero-native-dependency posture for an API Microsoft itself calls a "design limitation." See T7.1's full write-up and sources. |
...
| ID   | Task                                                             | Status |
| ---- | ---------------------------------------------------------------- | ------ |
| T7.1 | Investigate WMI-ordinal to Electron `Display.id` correlation     | done   |
...
**Done — investigation only, no code change.** Honesty constraint up front: this machine has no touch-capable display attached, so nothing below is an empirical hardware observation — it is desk research against Microsoft's own documentation, cited below. Treat the recommendation accordingly.

Researched (via live web search, sources below) whether any Windows API correlates a touch/pen digitizer to a specific monitor:

- **WMI's `Win32_PointingDevice`** (what this probe already uses) has no display-association property or paired association class at all. There is nothing to fix here — the class was never designed to expose this.
- **Windows' real digitizer-to-display mapping** is a driver/firmware-level mechanism, not a simple ordinal: external displays are matched by a shared **Container ID** (a GUID, reported via an EDID extension or a `DXGKDDI_GET_CHILD_CONTAINER_ID` driver callback); internal displays are matched via **ACPI `_PLD`** (physical location) fields. Neither has any relationship to WMI's enumeration order — a WMI ordinal position and a Container ID are answered by two unrelated subsystems, so the current probe's ordinals lining up with `Display.id` on any given machine would be coincidence, not a proven mapping. ([Digitizer Display Mapping — Microsoft Learn](https://learn.microsoft.com/en-us/windows-hardware/design/component-guidelines/digitizer-display-mapping))
- **A real, application-facing Win32 API for this does exist**: `POINTER_DEVICE_INFO` (returned by `GetPointerDevices`) includes an `HMONITOR` field — this, not WMI, is the actual intended mechanism for an application to query digitizer→monitor association. But it is native Win32 (would require a native binding, not a spawned `powershell.exe`/WMI call — the same category of cost this project already declined to pay in T2.13 for a comparable native-Win32-Job-Object option, for the same reason: no good zero-native-dependency path). Even using the _correct_ API, Microsoft's own community answer describes it as heuristic and unreliable in practice (misreports when a display is disabled, falls back to the primary display, "a design limitation... that has existed for over a decade"). ([How to handle touch device mapping in multi-monitor setups — Microsoft Q&A](https://learn.microsoft.com/en-us/answers/questions/2113548/how-to-handle-touch-device-mapping-in-multi-monito))
- Electron's own `touchSupport` is therefore not just the pragmatic choice but very likely already backed by Chromium's own internal use of this same (or an equivalent, more-authoritative) native signal — consistent with T2.12's finding that it should always win over this probe wherever it expresses an opinion.

**Recommendation: keep the probe, but treat its numbers as strictly experimental, keep it `enabled: false` by default (already the case), and do not invest further engineering into it.** A real fix (native `GetPointerDevices`/`HMONITOR` bindings) would trade a zero-native-dependency package for an API Microsoft itself documents as unreliable even when used correctly — not a good trade for a code path that, post-T2.12, only ever breaks a tie for a single `'unknown'`-`touchSupport` display. Removing the probe outright is also defensible (it was already considered), but not clearly better than the status quo: the code's own module doc comment (`src/layout/probes/windows-touch.ts`) already states this limitation as plainly as this report does, `TouchProbe` stays a clean interface other platforms/future probes can implement, and disabling by default already fully contains the risk. No source change made beyond this write-up — Open decision #10 (below) is updated to reflect this as resolved-by-research rather than still-open.

Sources:

- [Digitizer Display Mapping — Microsoft Learn](https://learn.microsoft.com/en-us/windows-hardware/design/component-guidelines/digitizer-display-mapping)
- [How to handle touch device mapping in multi-monitor setups using Windows API? — Microsoft Q&A](https://learn.microsoft.com/en-us/answers/questions/2113548/how-to-handle-touch-device-mapping-in-multi-monito)
- [Win32_PointingDevice class — Microsoft Learn](https://learn.microsoft.com/en-us/windows/win32/cimwin32prov/win32-pointingdevice)
